...
# original PHP ECS) running the same psr12 + common default set over a real
# codebase. All binaries do identical work, so times are comparable. Each binary
# fixes a fresh copy RUNS times; the mean wall time is reported.
#
# Env:
#   GO_BIN    path to the ecs-go binary
#   RUST_BIN  path to the ecs-rust binary
#   CONFIG    optional ecs-go.json for the Go side; unset runs the default set
...
echo "## ecs-go vs ecs-rust (psr12 + common default set, mean of $RUNS runs)"
echo ""
if [ -n "$ECS_CMD" ]; then

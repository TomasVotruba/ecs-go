...
// Fixers that are ported but not part of the psr12 + common default set (see
// ecs-go set.Default). Kept implemented and callable, but off by default so the
// Rust default output matches ECS/ecs-go instead of over-fixing.
const INCLUDE_NON_DEFAULT: bool = false;

// Rules run in ecs-go's rules.All() order, restricted to the psr12 + common
// default members, so the Rust default output matches the Go default output.
pub fn fix(s: &mut Stream) -> bool {
    let mut changed = false;
    changed |= encoding(s);
    changed |= full_opening_tag(s);
    changed |= protected_to_private(s);
    if INCLUDE_NON_DEFAULT { changed |= ordered_class_elements(s); }
    ...
    if INCLUDE_NON_DEFAULT { changed |= no_binary_string(s); }
    changed |= no_unneeded_braces(s);
    if INCLUDE_NON_DEFAULT { changed |= no_alias_functions(s); }
    changed |= remove_useless_default_comment(s);
    changed |= elseif(s);
    changed |= standalone_line_promoted_property(s);
    changed |= array_list_item_newline(s);
    if INCLUDE_NON_DEFAULT { changed |= empty_loop_body(s); }
    ...
    if INCLUDE_NON_DEFAULT { changed |= simplified_null_return(s); }
    if INCLUDE_NON_DEFAULT { changed |= increment_style(s); }
    changed |= single_quote(s);
    if INCLUDE_NON_DEFAULT { changed |= clean_namespace(s); }
    changed |= phpdoc_return_self_reference(s);
    if INCLUDE_NON_DEFAULT { changed |= fully_qualified_strict_types(s); }
    changed |= explicit_string_variable(s);
    changed |= no_superfluous_phpdoc_tags(s);
    if INCLUDE_NON_DEFAULT { changed |= phpdoc_no_useless_inheritdoc(s); }
    changed |= no_useless_concat_operator(s);
    changed |= phpdoc_line_span(s);
    if INCLUDE_NON_DEFAULT { changed |= standalone_line_in_multiline_array(s); }
    ...
    if INCLUDE_NON_DEFAULT { changed |= nullable_type_declaration(s); }
    changed |= function_to_constant(s);
    if INCLUDE_NON_DEFAULT { changed |= linebreak_after_opening_tag(s); }
    changed |= is_null(s);
    changed |= single_line_comment_spacing(s);
    if INCLUDE_NON_DEFAULT { changed |= no_space_around_double_colon(s); }
    changed |= no_unneeded_import_alias(s);
    if INCLUDE_NON_DEFAULT { changed |= empty_loop_condition(s); }
    if INCLUDE_NON_DEFAULT { changed |= doctrine_annotation_array_assignment(s); }
    ...
    if INCLUDE_NON_DEFAULT { changed |= attribute_block_no_spaces(s); }
    if INCLUDE_NON_DEFAULT { changed |= heredoc_to_nowdoc(s); }
    if INCLUDE_NON_DEFAULT { changed |= no_unset_cast(s); }
    changed |= no_whitespace_in_empty_array(s);
    if INCLUDE_NON_DEFAULT { changed |= normalize_index_brace(s); }
    ...
    if INCLUDE_NON_DEFAULT { changed |= compact_nullable_type_declaration(s); }
    if INCLUDE_NON_DEFAULT { changed |= ordered_types(s); }
    changed |= explicit_indirect_variable(s);
    changed |= no_null_property_initialization(s);
    changed |= include(s);
    changed |= add_missing_param_name(s);
    if INCLUDE_NON_DEFAULT { changed |= no_alias_language_construct_call(s); }
    if INCLUDE_NON_DEFAULT { changed |= no_unreachable_default_argument_value(s); }
    if INCLUDE_NON_DEFAULT { changed |= doctrine_annotation_spaces(s); }
    if INCLUDE_NON_DEFAULT { changed |= doctrine_annotation_indentation(s); }
    ...
    if INCLUDE_NON_DEFAULT { changed |= phpdoc_types_order(s); }
    if INCLUDE_NON_DEFAULT { changed |= phpdoc_var_annotation_correct_order(s); }
    if INCLUDE_NON_DEFAULT { changed |= phpdoc_summary(s); }
    if INCLUDE_NON_DEFAULT { changed |= phpdoc_tag_type(s); }
    changed |= lowercase_keywords(s);
    changed |= constant_case(s);
    if INCLUDE_NON_DEFAULT { changed |= lowercase_static_reference(s); }
    ...
    if INCLUDE_NON_DEFAULT { changed |= native_function_type_declaration_casing(s); }
    ...
    if INCLUDE_NON_DEFAULT { changed |= multiline_whitespace_before_semicolons(s); }
    ...
    if INCLUDE_NON_DEFAULT { changed |= standalone_line_plain_constructor_param(s); }
    changed |= standalone_line_required_param(s);
    if INCLUDE_NON_DEFAULT { changed |= standalone_line_symfony_attribute_param(s); }
    ...
    if INCLUDE_NON_DEFAULT { changed |= ordered_interfaces(s); }
    if INCLUDE_NON_DEFAULT { changed |= ordered_traits(s); }
    if INCLUDE_NON_DEFAULT { changed |= phpunit_method_casing(s); }
    if INCLUDE_NON_DEFAULT { changed |= phpunit_setup_teardown_visibility(s); }
    if INCLUDE_NON_DEFAULT { changed |= operator_linebreak(s); }
    if INCLUDE_NON_DEFAULT { changed |= phpdoc_separation(s); }
    if INCLUDE_NON_DEFAULT { changed |= phpdoc_to_comment(s); }
    changed |= param_return_and_var_tag_malforms(s);
    if INCLUDE_NON_DEFAULT { changed |= general_phpdoc_tag_rename(s); }
    changed |= visibility_required(s);
    if INCLUDE_NON_DEFAULT { changed |= modifier_keywords(s); }
    ...
    if INCLUDE_NON_DEFAULT { changed |= self_static_accessor(s); }
    ...
    if INCLUDE_NON_DEFAULT { changed |= single_line_empty_body(s); }
    ...
    if INCLUDE_NON_DEFAULT { changed |= blank_line_before_statement(s); }
    changed |= ordered_imports(s);
    if INCLUDE_NON_DEFAULT { changed |= single_line_comment_style(s); }
    if INCLUDE_NON_DEFAULT { changed |= blank_lines_before_namespace(s); }
    ...
    if INCLUDE_NON_DEFAULT { changed |= blank_line_between_import_groups(s); }
    changed |= phpdoc_trim_consecutive_blank_line_separation(s);
    if INCLUDE_NON_DEFAULT { changed |= phpdoc_align(s); }
